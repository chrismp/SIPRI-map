...
function makeTooltip(layer,data,dataType,year){
	...
	var tooltipHTML = tooltipInfoArray.map(function(elem,idx){
		...
		if(idx===0){
			...
			txt = 'Weapons imports: $'+numberWithCommas(elem);
		}
		// var txt = idx===0 ? '<b>'+elem+'</b>' : 'Weapons imports: $'+numberWithCommas(elem)+'m';
		return '<p>'+txt+'</p>';
	}).join('');
...
window.onload = function(){
	...
	cartodb.createLayer(mapObject,layerSource)
		...
		.on('done',function(layer){
			...
			sublayer.setInteractivity('cntry_name, importyear1950');
			sublayer.on('featureOver', function(e, latlng, pos, data, subLayerIndex) {
				makeTooltip(layer,data,'importyear',1950);
			...
			$('#ex1').on('slide',function(slideEvent){
				...
				sublayer.setSQL("SELECT * FROM "+cartoDbTableName+" WHERE (importyear"+year+">0 AND gwsyear<="+year+" AND gwsyear>0)");
				sublayer.setCartoCSS(
					"#sipri_import_import_map_1950_2014{" +
					  "polygon-fill: #dddddd;" +
					  "polygon-opacity: 1;" +
					  "line-color: #FFF;" +
					  "line-width: 0.5;" +
					  "line-opacity: 1;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" > 1000] {" +
					   "polygon-fill: #B10026;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 1000] {" +
					   "polygon-fill: #E31A1C;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 800] {" +
					   "polygon-fill: #FC4E2A;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 600] {" +
					   "polygon-fill: #FD8D3C;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 400] {" +
					   "polygon-fill: #FEB24C;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 200] {" +
					   "polygon-fill: #FED976;" +
					"}" +
					"#sipri_import_import_map_1950_2014 [ importyear"+year+" <= 20] {" +
					   "polygon-fill: #FFFFB2;" +
					"}");
				
				sublayer.setInteractivity('cntry_name, importyear'+year);
				
				sublayer.on('featureOver', function(e, latlng, pos, data, subLayerIndex) {
					makeTooltip(layer,data,'importyear',year);
				}).on('error',function(err){
